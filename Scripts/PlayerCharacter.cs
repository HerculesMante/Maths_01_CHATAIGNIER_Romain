...
public class PlayerCharacter : MonoBehaviour
{
    ...
    private bool _gravityReverse = false;
    ...
    private bool autojumping = false;
    ...
    private void Movement()
    {
        ...
        {
            float maxSpeed = _horizontalPhysic.MaxSpeed * _movementInput * 2;
            float velocityDot = Mathf.Clamp(_rigidbody.velocity.x * maxSpeed, -1.5f, 1.5f);
            ...
            _forceToAdd.x += velocityDelta * 1.5f;
        }
    ...
    private void Gravity()
    {
        ...
        else if (_gravityReverse == true)
        {
            float coyoteTimeRatio = Mathf.Clamp01(_airTime / _gravityParameters.CoyoteTime);
            float coyoteTimeFactor = _isInCoyoteTime ? _gravityParameters.GravityRemapFromCoyoteTime.Evaluate(coyoteTimeRatio) : 1.0f;
            float acceleration = _gravityParameters.Acceleration * coyoteTimeFactor * Time.fixedDeltaTime * 0.1f;

            _currentGravity = Mathf.MoveTowards(_currentGravity, _gravityParameters.MaxForce, (acceleration * 0.1f));
    ...
    {
        if (collsion.gameObject.GetComponent<AutoJump>() && autojumping == false)
        {
            StartJump();
            autojumping = true;
        }

        else if (collsion.gameObject.GetComponent<GravityReverse>())
        {
            //INverse la gravité
            Debug.Log("inverseGravité");
            _gravityReverse = !_gravityReverse;
        }
    ...
    {
        Debug.Log("Sprint");
        _isSprinting = !_isSprinting; //Inverse le bool à chaque fois que ActionOne est appeler
    }
